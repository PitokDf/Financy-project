...
class GamificationService {
    constructor(gamificationRepo) {
        ...
        this.XP_TRANSACTION = 10;
        this.XP_ANALYSIS = 50;
        this.XP_EXPORT = 20;
        this.awardXP = async (userId, amount, action) => {
        ...
        this.syncStreak = async (userId) => {
            const stats = await this.gamificationRepo.getStats(userId);
            if (!stats || !stats.lastTransactionAt)
                return stats;
            const now = new Date();
            const diffDays = (0, date_fns_1.differenceInCalendarDays)(now, stats.lastTransactionAt);
            if (diffDays > 1) {
        ...
        this.updateStreak = async (userId) => {
            const stats = await this.gamificationRepo.getStats(userId);
            if (!stats)
                return this.awardXP(userId, this.XP_TRANSACTION, "FIRST_TRANSACTION");
            ...
            if (!lastTx) {
                ...
                const diffDays = (0, date_fns_1.differenceInCalendarDays)(now, lastTx);
                if (diffDays === 1) {
                    newStreak += 1;
                }
                else if (diffDays > 1) {
            ...
            if (newStreak === 3)
                await this.checkAndAwardBadge(userId, "streak_3");
            if (newStreak === 7)
                await this.checkAndAwardBadge(userId, "streak_7");
            if (newStreak === 30)
                await this.checkAndAwardBadge(userId, "streak_30");
            return { streak: newStreak };
        };
        this.checkAndAwardBadge = async (userId, condition) => {
        ...
        this.ensureWeeklyChallenges = async (userId) => {
            const now = new Date();
            ...
            const allChallenges = await prisma_1.default.challenge.findMany();
            ...
            const userChallenges = await Promise.all(selected.map(challenge => prisma_1.default.userChallenge.create({
                data: {
                    userId,
        ...
        this.updateChallengeProgress = async (userId, type, amount = 1) => {
            await this.ensureWeeklyChallenges(userId);
            const now = new Date();
            const weekNumber = this.getWeekNumber(now);
            const year = now.getFullYear();
            const activeChallenges = await prisma_1.default.userChallenge.findMany({
                where: {
                    userId,
                    isCompleted: false,
                    weekNumber,
            ...
            for (const uc of activeChallenges) {
                ...
                await prisma_1.default.userChallenge.update({
                    where: { id: uc.id },
                    data: {
                        current: newProgress,
