...
export class AnalysisService {
  ...
  public getLatestRun = async (userId: string) => {
    ...
    return {
      ...
      clusters: run.clusters.map((cluster) => ({
        ...
        totalAmount:
          (cluster as any).transactions?.reduce(
            (sum: number, t: any) => sum + Math.abs(Number(t.amount)),
            0,
          ) || 0,
        representativeDescriptions:
          cluster.suggestedName && cluster.suggestedName !== "Unknown"
            ? [cluster.suggestedName]
            : [],
        members:
          (cluster as any).transactions?.map((t: any) => ({
            id: t.id,
  ...
  public getCategoryBreakdown = async (
    userId: string,
    startDate?: string,
    endDate?: string,
  ) => {
    ...
    return categories
      .map((cat: any) => ({
        ...
        name: cat.name,
        color: cat.color,
        totalAmount: cat.transactions.reduce(
          (sum: number, t: any) => sum + Math.abs(Number(t.amount)),
          0,
        ),
      }))
  ...
  public run = async (payload: RunAnalysisInput, userId: string) => {
    ...
    const transactions = await this.transactionRepo.findForAnalysis(
      userId,
      startDate,
      endDate,
    );
    ...
    try {
      ...
      logger.info(
        `Detected ${mlResult.preAssigned.length} transactions for pre-assignment. Deferring update until confirmation.`,
      );
      ...
      for (const cluster of mlResult.clusters) {
        ...
        const transactionIds = cluster.members.map(
          (member) => member.transaction_id,
        );
        await this.transactionRepo.updateCluster(
          transactionIds,
          createdCluster.id,
        );
      ...
      return {
        ...
        preAssignedSummary: {
          ...
          byCategory: mlResult.preAssigned.reduce(
            (acc, pa) => {
  ...
  public runV2 = async (payload: RunAnalysisInput, userId: string) => {
    ...
    try {
      ...
      logger.info(
        `[V2] Model: ${mlResult.modelVersion} | Predictions: ${mlResult.predictions.length} | Low-confidence: ${mlResult.reviewCount}`,
      );
      ...
      for (const [categoryName, members] of grouped.entries()) {
        const isLainLain =
          categoryName.toLowerCase() === "lain-lain" ||
        ...
        const totalAmount = detailedMembers.reduce(
          (sum: number, m: any) => sum + Math.abs(m.amount),
          0,
        );
      ...
      await PushService.sendNotificationToUser(
        userId,
        "Clustering Selesai!",
        "Hasil sudah siap untuk ditinjau.",
        { url: "/analysis/lab" },
      );
  ...
  public confirm = async (payload: ConfirmAnalysisInput) => {
    const run = await this.analysisRepo.findRunWithClusters(
      payload.runId,
      payload.userId,
    );
    ...
    if (preAssigned.length > 0) {
      ...
      for (const [categoryId, txIds] of preAssignMap.entries()) {
        await this.transactionRepo.updateCategoryByTransactionIds(
          txIds,
          categoryId,
        );
      }
      logger.info(
        `Confirmed pre-assignment for ${preAssigned.length} transactions.`,
      );
    }

    const analysisMeta = (run.wcssValues as any).clusters ?? [];
    const allTransactions = run.clusters.flatMap(
      (c) => (c as any).transactions || [],
    );
    ...
    for (const mapping of payload.clusterMappings) {
      const clusterMeta = analysisMeta.find(
        (cluster: any) => cluster.index === mapping.index,
      );
      const cluster = run.clusters.find(
        (cluster: any) => cluster.index === mapping.index,
      );
      ...
      if (
        !cluster &&
        mapping.index !== -1 &&
        (!mapping.transactionIds || mapping.transactionIds.length === 0)
      ) {
        throw new AppError(
          `Cluster index ${mapping.index} tidak valid (bukan hasil AI dan kosong)`,
          HttpStatus.NOT_FOUND,
        );
      }

      const transactionIds =
        mapping.transactionIds ||
    ...
    if (payload.reviewOverrides && payload.reviewOverrides.length > 0) {
      for (const override of payload.reviewOverrides) {
        let targetCategoryId = override.categoryId;

        if (!targetCategoryId && override.categoryName) {
          const t = transactionMap.get(override.transactionId);
          const category = await this.categoryRepo.upsertByUserAndName({
            userId: payload.userId,
            name: override.categoryName,
            type: t?.type === "INCOME" ? "INCOME" : "EXPENSE",
            color: CategoryMap[override.categoryName]?.color ?? "#888888",
            icon: CategoryMap[override.categoryName]?.icon ?? "Ellipsis",
            isAutoGenerated: true,
            aiConfidence: run.silhouetteScore ?? null,
            aiKeywords: [override.categoryName.toLowerCase()],
          });
      ...
      logger.info(
        `Applied ${payload.reviewOverrides.length} review overrides for run ${run.id}.`,
      );
    }

    const topForecasts = await this.forecastService.getTopForecasts(
      payload.userId,
      3,
    );
