...
export class AuthService {
    ...
    public forgotPassword = async (email: string) => {
        ...
        if (!user) throw new AppError('Email tidak ditemukan', HttpStatus.NOT_FOUND);

        if (!user.password) {
            throw new AppError('Akun ini terdaftar via Google SSO. Silakan masuk dengan Google.', HttpStatus.BAD_REQUEST);
        }
        ...
        try {
            ...
            logger.error('[Auth] Failed to send reset email, but token was created:', error);
        }
    ...
    public resetPassword = async (token: string, newPassword: string) => {
        ...
        if (!resetToken) throw new AppError('Token tidak valid', HttpStatus.BAD_REQUEST);
        if (resetToken.used) throw new AppError('Token sudah digunakan', HttpStatus.BAD_REQUEST);
        if (resetToken.expiresAt < new Date()) throw new AppError('Token sudah kedaluwarsa. Silakan minta reset password lagi.', HttpStatus.BAD_REQUEST);
